...
int virtio_mmio_init(virtio_mmio_dev_t *dev, uint32_t device_num)
{
	...
	/* Reset device — STATUS is a 32-bit register in MMIO v2 */
...
{
	DEBUGASSERT(dev && dev->initialized);
	virtio_mmio_write32(dev, VIRTIO_MMIO_STATUS, (uint32_t)status);
}
...
/****************************************************************************
 * Name: virtio_mmio_setup_queue_v1
 *
 * Description:
 *   Register a virtqueue with a v1 (legacy) MMIO device using QUEUE_PFN.
 *   The queue memory base (vq->desc) must be 4096-byte aligned.
 *
 ****************************************************************************/
...
{
	DEBUGASSERT(dev && dev->initialized);
	DEBUGASSERT((queue_base_pa & 0xFFF) == 0);	/* must be page-aligned */

	virtio_mmio_write32(dev, VIRTIO_MMIO_GUEST_PAGE_SIZE, 4096);
	virtio_mmio_write32(dev, VIRTIO_MMIO_QUEUE_SEL, queue_sel);
	virtio_mmio_write32(dev, VIRTIO_MMIO_QUEUE_NUM, queue_num);
	virtio_mmio_write32(dev, VIRTIO_MMIO_QUEUE_ALIGN, 4096);
	virtio_mmio_write32(dev, VIRTIO_MMIO_QUEUE_PFN, (uint32_t)(queue_base_pa >> 12));

	return OK;
}

/****************************************************************************
 * Name: virtio_mmio_setup_queue
 *
 * Description:
 *   Register a virtqueue with the MMIO device (v2 transport)
 *
 ****************************************************************************/

int virtio_mmio_setup_queue(virtio_mmio_dev_t *dev, uint32_t queue_sel,
			    uint32_t queue_num, uint64_t desc_addr,
