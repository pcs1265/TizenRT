...
|---------------|-------------|-----|---------------|-------------|
| `dramboot_flat` | Flat (`CONFIG_BUILD_FLAT=y`) | Single Core | - | Simple build model where OS and applications are linked into a single binary |
| `dramboot_flat_smp` | Flat (`CONFIG_BUILD_FLAT=y`) | SMP (4 cores) | - | SMP-enabled flat build |
...
- **virtio-net**: Network device driver implementation
